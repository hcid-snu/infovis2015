...
// 어떤 도법을 사용할 것인가 지정해주는 부분 
	...
	.center([126.992484, 37.563275]) // 지도의 중심이 될 좌표를 설정
	.scale(110000)
...
function ready(error, data) {
	...
	korea.selectAll("path")
		...
		.attr("class", function(d) {
			if(d.properties.code.charAt(0) == '1') {
				return "municipality " + d.properties.code
			}
			else{
				return "nonSelected " + d.properties.code	
				// return 'nonSelected'	
			}
		})
		.attr("d", path)
		...
		.on('mouseover', function(d,i){
			if(!korea.selectAll('text' + '.c' + d.properties.code).classed('show') && d.properties.code.charAt(0) == '1'){
