...
queue()
    ...
    .defer(d3.csv, "data/obesity.csv", function(d) { popByName.set(d.name, +d.value) })
    .await(ready)
...
function ready(error, data) {
  var features = topojson.feature(data, data.objects.seoul_municipalities_geo).features
	...
	var colorScale = d3.scale.quantize()
			.domain([12, 16])
			.range(d3.range(4).map(function(i) { return "p" + i }))
  ...
  seoul.selectAll("path")
	...
	seoul.selectAll("text")
			...
			.attr("dy", ".35em")
			.text(function(d) { return d.properties.name + " " + popByName.get(d.properties.name) })
