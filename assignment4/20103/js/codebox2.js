...
d3.csv("data/mydata_agent.csv", function(error, data){
  ...
  // svg 요소의 크기를 구한다
  var svgEle = document.getElementById("agentPie2")
  ...
  // 데이터 로딩
  var dataSet_sm = []  // empty array to store data
  var dataLabel_sm = []  // empty array to store label
  for(var i = 0; i < data.length; i++) {
    dataSet_sm.push(data[i].sm)
    dataLabel_sm.push(data[i].Agent)
  }

  console.log(dataSet_sm)
  ...
  // pie chart 그리기 준비
  var pieElements = d3.select("#agentPie2")
    .selectAll("g")
    .data(pie(dataSet_sm))
    .enter()
    .append("g")
  ...
  // 파이 데이터의 총 합을 표시
  var textElements = d3.select("#agentPie2")
    .append("text")
    .attr("class", "total")
